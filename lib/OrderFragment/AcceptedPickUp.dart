...
import 'package:spicy_food_delivery/OrderFragment/pickupDetails.dart';
import 'package:spicy_food_delivery/Utils/Const.dart';
import 'package:spicy_food_delivery/api/pickupListingApi.dart';

class AcceptedPickUp extends StatefulWidget {
  @override
  _ListTileItemState createState() => new _ListTileItemState();
}

class _ListTileItemState extends State<AcceptedPickUp> {
  var isLoading = true;
  ...
  Future<String> getProducts() async {
    ...
    var rsp = await pickupListingApi();
    print("pickkkkkkkkkkkkkkkk");
    print(rsp);
    if (rsp != 0) {
      arrList.clear();
      arrProdList.clear();
      setState(() {
        arrList = rsp['pickAndDrops'];
      });
      for (var i = 0; i < arrList.length; i++) {
        if (arrList[i]['pickUpStatus'] == 2) {
          arrProdList.add(arrList[i]);
        }
        if (arrList[i]['pickUpStatus'] == 3) {
          arrProdList.add(arrList[i]);
        }
    ...
    return GestureDetector(
      onTap: () async {
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => PickupDetailsPg(
                    data: item,
                  )),
        );
  ...
  //       content: new Text(
  //           'Did you want to change the status !'),
  //       actions: <Widget>[
  //         new GestureDetector(
